...
void getHiddenString(char stringToGet[KEY_LENGTH_SIZE_WITH_ZERO])
{
    ...
    if (echoCharacterIndex >= 0)
        stringToGet[echoCharacterIndex] = 0;
...
int userPromptWithArg(const char* message, char* arg)
{
    ...
    c = tolower(c & 0xFF);
#if defined(DEBUG)
    printf("DEBUG: [userPromptWithArg] User pressed: [%d] = "
           "%c\n",
           c, c);
#endif // defined(DEBUG)
    return c;
...
void getPasswordWithConfirmation(char passwd[KEY_LENGTH_SIZE_WITH_ZERO])
{
    ...
    } while (strlen(passwdConfirm) < MIN_KEY_LENGTH_SIZE ||
             strcmp(passwdConfirm, passwd) != 0);

#if defined(DEBUG)
    printf("DEBUG [getPasswordWithConfirmation]: Password: "
           "[%s]\n",
           passwd);
#endif // defined(DEBUG)
}
...
#define updateProgress(current, full)                     \
    printf("DEBUG [updateProgress]: Current Progress at " \
           "%ld/%ld.\n",                                  \
           current, full);
